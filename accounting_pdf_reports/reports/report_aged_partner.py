...
class ReportAgedPartnerBalance(models.AbstractModel):
    ...
    def _get_partner_move_lines(self, account_type, date_from, target_move, period_length):
        ...
        #construire la clause de réconciliation pour voir quel partenaire doit être imprimé
        ...
        # mettre un total de 0
        for i in range(7):
            total.append(0)

        # Construisez une chaîne comme (1,2,3) pour une utilisation facile dans la requête SQL
        partner_ids = [partner['partner_id'] for partner in partners if partner['partner_id']]
        lines = dict((partner['partner_id'] or False, []) for partner in partners)
        if not partner_ids:
            return [], [], {}

        # Ce dictionnaire stockera le montant non dû de tous les partenaires
        undue_amounts = {}
        query = '''SELECT l.id
